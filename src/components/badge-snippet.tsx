...
export function BadgeSnippet({ slug, score }: { slug: string; score: number }) {
  ...
  const snippet = `<a href="${pageUrl}" title="Agent-readiness score by Qomvia" target="_blank" rel="noopener"><img src="${badgeUrl}" alt="Qomvia agent-readiness score ${score}/100" width="${BADGE_WIDTH}" height="${BADGE_HEIGHT}"></a>`;
  return (
