...
| Public result page | Live | `/site/<slug>`: a verdict sentence, blocker/weak-spot/passed counts, the five findings that cost the shop most (title and consequence only) and agent compatibility. The check-by-check list stops at a blurred placeholder panel — the detail is never sent to the page — and opens in the dashboard after a free signup; fix instructions and evidence stay in the paid report. |
...
| Dashboard | Live | `/dashboard` (noindex): readiness status, every check with what the scan found, catalogue, visibility, competitors, credits. |
...
| Attached stores | Live | A signed-in account attaches any domain permanently. `owned` needs a code mailed to an administrative address at the domain (admin@, webmaster@, info@, …) and unlocks re-scans, the fix report and the seal; `watched` needs no proof because everything measured is public. First watched domain free, then one CHF 19/month competitor slot each. |
| Catalogue import jobs | Live | Attaching queues a resumable `ImportJob`: source discovery (Shopify `/products.json` → XML feed → product sitemap + JSON-LD), then fetch, parse and upsert, with state, counts and last error shown in the dashboard. One bounded pass runs when the store is attached, the rest is drained by `/api/cron/imports` (daily on Vercel Hobby, which allows one cron a day) and retried up to three times before the job reports what it could not read. A page-level pass reads JSON-LD first and falls back to the page's own title/price markup, so a shop with no structured data still imports. Resume from the dashboard. A watched catalogue is sampled at 200 products, an owned one at 2,000. |
